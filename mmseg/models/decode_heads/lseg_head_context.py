...
class TextEncoder(nn.Module):
    def __init__(self, clip_model, embed_dim):
        ...
        self.text_projection = nn.Parameter(
            torch.empty(clip_model.transformer.width, embed_dim))
...
class PromptLearner(nn.Module):
    def __init__(self, n_ctx, classnames, ctx_init, clip_model, vis_dim):
        ...
        tokenized_prompts = torch.cat(
            [clip.tokenize(p) for p in prompts])  # (n_cls, n_tkn)
        with torch.no_grad():
...
class LsegHeadContext(BaseDecodeHead):
    def __init__(self, class_names, tau, arch_option, block_depth, activation, input_resolution, n_ctx, ctx_init, **kwargs):
        ...
        self.prompt_learner = PromptLearner(
            n_ctx, self.class_names, ctx_init, clip_model, final_embed_dim)
        ...
        self.linear_image = nn.Conv2d(
            final_embed_dim, final_embed_dim, input_resolution // 4)

        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
        # self.convproj = nn.Conv2d(self.channels + self.num_classes, self.channels, kernel_size=3, padding=1)
    ...
    def forward(self, inputs):
        ...
        img_vector = self.linear_image(x).view(n, -1)
